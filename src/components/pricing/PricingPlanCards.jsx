...
function PricingCards() {
  return <div>
    ...
    <div className="grid grid-cols-1 mt-5 sm:grid-cols-2 xl:grid-cols-4 gap-10  mx-auto max-w-xl  sm:max-w-6xl xl:max-w-[2000px]">
      {plans.map((plan, i) => (
        <FadeInSection animation="blur" delay={0.3} key={i} className={`bg-black/90 rounded-2xl p-8 justify-center flex-col flex border hover:scale-[1.04] transition-all duration-600 delay-100 linear  ${plan.highlight
          ...
          >
            ...
            <ul className="mt-6 space-y-3 text-text/80 leading-relaxed tracking-wide text-[1.35rem]">
              {plan.features.map((f, i) => (
                <li key={i} className="flex items-center gap-3">
                  <HiCheck className={`text-[1.5rem] ${plan.name === "Starter Plan" ? "text-gray-400" :
            ...
            <button
              className={`w-full mt-10 py-3 rounded-lg text-[1.3rem] font-semibold transition-all duration-500 uppercase font-montserrat tracking-wide ${plan.highlight
